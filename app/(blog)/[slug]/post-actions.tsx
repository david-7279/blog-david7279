...
export function PostActions({ slug }: { slug: string }) {
  ...
  // Carrega stats + view + voto guardado
  // Carrega stats + view (só uma vez por dispositivo) + voto guardado
  useEffect(() => {
    async function init() {
      const viewKey = `viewed:${slug}`;
      const alreadyViewed = localStorage.getItem(viewKey);

      // Só incrementa se ainda não viu neste dispositivo
      if (!alreadyViewed) {
      ...
      // Busca sempre as stats atualizadas
      const res = await fetch(`/api/views?slug=${slug}`);
      const data = await res.json();
      setStats(data);

      // Lê voto do localStorage
  ...
  async function handleVote(type: "up" | "down") {
    ...
    // Se clicou no mesmo → remove o voto
    if (userVote === type) {
      next = null;
    }
